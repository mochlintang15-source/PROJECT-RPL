                ...
                <div class="col-lg-5">
                    <div class="card border-0 shadow-sm rounded-4 p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="fw-bold"><?= e($order['nama'] ?: ($order['nama_user'] ?? 'User tidak ditemukan')) ?></div>
                            <?php if (!empty($order['email']) || !empty($order['email_user']) || !empty($order['no_hp'])): ?>
                                <div class="text-muted small"><?= e($order['email'] ?: ($order['email_user'] ?? '-')) ?><?= !empty($order['no_hp']) ? ' | ' . e($order['no_hp']) : '' ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3 p-3 rounded-4 bg-light border">
                            <div class="fw-semibold mb-2">Pembayaran</div>
                            <div class="row g-2 small">
                                ...
                                <div class="col-6">
                                    ...
                                    <div class="fw-semibold"><?= e(ucfirst($order['status_pembayaran'] ?? 'pending')) ?></div>
                                </div>
                                <div class="col-12">
                                    <div class="text-muted">Nomor Pembayaran</div>
                                    <div class="fw-semibold"><?= e($order['nomor_pembayaran'] ?? '-') ?></div>
                                </div>
                            </div>
                            <?php if (!empty($order['bukti_transfer'])): ?>
                                <?php $proofUrl = findPaymentProofUrl((string) $order['bukti_transfer']); ?>
                                <a href="<?= e($proofUrl) ?>" target="_blank" class="btn btn-outline-primary btn-sm rounded-3 mt-3"><i class="bi bi-image me-1"></i> Lihat Bukti Pembayaran</a>
                            <?php else: ?>
                                <div class="text-muted small mt-3">Belum ada bukti pembayaran.</div>
                            <?php endif; ?>
                        </div>
